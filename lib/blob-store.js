...
import { diffInstitution, newProgramme, LOCKED_INSTITUTION_FIELDS, LOCKED_PROGRAMME_FIELDS } from './db';
...
async function load() {
  ...
  // A deploy that ships a newer master dataset (new campuses, institutions or
  // programmes pulled from the catalogue) replaces the stored copy once; the
  // sync history is carried over. Later syncs keep updating the blob as usual.
  if (!ds || ds.seedVersion !== seedData.generatedAt) {
    ds = { ...fresh(), seedVersion: seedData.generatedAt, syncRuns: ds?.syncRuns || [], lastSyncedAt: ds?.lastSyncedAt || null };
    await writeBlob(ds);
  }
  cached = ds;
  return ds;
...
export async function applyInstitution(instId, catalogue, { log = () => {} } = {}) {
  ...
  inst.programs = inst.programs.map((p) => {
    ...
    // patch.after never carries slug/path — keep the share link stable
    return { ...base, ...patch.after, slug: base.slug, path: base.path, intakeList: sortIntakes(patch.after.intakeList), catalogueMissingSince: null, syncedAt: now };
  });
  // Programmes the catalogue lists that we do not have yet (a new course, or
  // a campus that started offering one) are added, tagged with their campus.
  const known = new Set(inst.programs.map((p) => p.portalId).filter(Boolean));
  const dest = ds.destinations.find((d) => d.institutions.includes(inst));
  const used = new Set(inst.programs.map((p) => p.slug));
  let added = 0;
  for (const c of catalogue.programs) {
    if (!c.portalId || known.has(c.portalId)) continue;
    const prog = newProgramme(c, inst, used, dest);
    inst.programs.push({ ...prog, syncedAt: now });
    known.add(c.portalId);
    added++;
    log(`   + ${c.name}${c.campus ? ` (${c.campus})` : ''}: new in the catalogue`);
  }
  inst.portalLogo = catalogue.logo || inst.portalLogo || null;
  inst.syncedAt = now;
  await save(ds);
  return { updated: updated + added, unchanged, missing: missing.length, added };
}
  ...
  ds.destinations = data.destinations.map((jd) => {
    ...
    return {
      ...
      institutions: jd.institutions.map((ji) => {
        ...
        for (const k of ['name', 'campus', 'city', 'type', 'logo', 'contact', 'progTags', 'portalCountryId', 'portalUniId', 'catalogueSource', 'slug']) inst[k] = ji[k];
        ...
        inst.programs = ji.programs.map((jp) => {
          ...
          prog.ptype = jp.ptype;
          if (jp.slug) prog.slug = jp.slug;
          if (jp.path) prog.path = jp.path;
