...
export function diffInstitution(existingPrograms, cataloguePrograms) {
  ...
  for (const p of existingPrograms) {
    ...
    // campus is only known when the catalogue was read campus by campus
    if (c.campus) { after.campus = c.campus; after.campusId = c.campusId; }
    const changed = [];
    const same = (x, y) => {
...
/** Level text -> 'pg' | 'ug', the split the level filter uses. */
export const ptypeFor = (level) =>
  /master|post ?graduate|postgraduate|graduate cert|graduate dip|phd|doctor|mba|pgd/i.test(level || '') ? 'pg' : 'ug';

const slugify = (s) =>
  String(s || '').normalize('NFKD').replace(/[̀-ͯ]/g, '').replace(/&/g, ' and ').replace(/['’]/g, '')
    .toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 80).replace(/-+$/g, '');

/**
 * A programme record for a catalogue card we did not have before. The slug is
 * the programme name; when another campus already uses it, the campus is
 * appended so every campus's version gets its own share link.
 */
export function newProgramme(c, inst, usedSlugs, dest) {
  let slug = slugify(c.name);
  if (usedSlugs.has(slug) && c.campus) slug = slugify(`${c.name} ${c.campus.replace(/campus/i, '')}`);
  const base = slug; let n = 2;
  while (usedSlugs.has(slug)) slug = `${base}-${n++}`;
  usedSlugs.add(slug);
  const intakeList = sortIntakes(c.intakeList || []);
  return {
    id: `${inst.id}-${slug}`,
    portalId: c.portalId || null,
    name: c.name,
    ptype: ptypeFor(c.level),
    level: c.level || null,
    ...
    slug,
    path: `/${dest?.code || 'x'}/${inst.slug || inst.id}/${slug}`,
  };
}
