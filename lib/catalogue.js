...
export const UNI = {
  ...
  'niagara-college': [1, 118], lethbridge: [1, 70], 'red-river': [1, 148],
...
/**
 * The catalogue lists every campus as its own "university". Read each campus of
 * one institution and return a single catalogue whose programmes carry the
 * campus they belong to. `campuses` is [{ portalUniId, name }].
 */
export async function fetchCampuses(cId, campuses, { pace = 1500, ...opts } = {}) {
  const out = { logo: null, total: 0, programs: [], campuses: [] };
  for (let i = 0; i < campuses.length; i++) {
    const c = campuses[i];
    const page = await fetchInstitution(cId, c.portalUniId, { pace, ...opts });
    out.logo = out.logo || page.logo;
    out.total += page.total ?? page.programs.length;
    out.campuses.push({ ...c, count: page.programs.length });
    for (const p of page.programs) out.programs.push({ ...p, campus: c.name, campusId: c.portalUniId });
    out.source = page.source;
    if (i < campuses.length - 1) await sleep(pace);
  }
  return out;
}
